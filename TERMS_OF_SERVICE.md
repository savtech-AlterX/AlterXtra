# Terms of Service for AlterX

**Last updated:** September 2026

## Overview

These terms govern your use of AlterX ("the app"), a personal identity-transformation and habit-tracking app. By using AlterX, you agree to these terms. If you don't agree, please don't use the app.

## The Service

AlterX helps you define an identity/archetype you're working toward and track your progress toward it — through a diary, future-self letters and videos, goals, habit-reprogramming entries, limited beliefs, a log book, quick notes, and photos/videos. As described in our [Privacy Policy](./PRIVACY_POLICY.md), everything you create stays on your device — AlterX has no server or account system.

## Not Professional Advice

AlterX is a self-guided personal development tool. It is not therapy, counseling, or medical advice, and it is not a substitute for professional mental health support. If you're in crisis or need to talk to someone, please reach out to a local emergency service or a crisis line in your country — for example, in the US and Canada, call or text **988** (Suicide & Crisis Lifeline); in the UK, call **116 123** (Samaritans).

## Your Content

Everything you create in AlterX — journal entries, letters, videos, photos, and everything else — is yours. AlterX doesn't claim any ownership over it, and since it's stored only on your device, we never see it. You're responsible for keeping your own backups (via the app's export feature or your device's iCloud/Google backup) — if your device is lost, damaged, or reset without a backup, that content cannot be recovered.

## Age Requirement

AlterX is not directed at children under 13, and you must be at least 13 years old to use it.

## In-App Purchases

If AlterX offers an optional one-time unlock (Alter-Xtra) or other in-app purchases, those are billed and fulfilled entirely by Apple's App Store or Google Play. Refunds, billing disputes, and payment issues are handled directly by Apple or Google under their own terms — AlterX does not process or store payment information.

## No Warranty

AlterX is provided "as is," without warranties of any kind, express or implied. We don't guarantee the app will be uninterrupted, error-free, or that it will achieve any particular personal outcome for you — your results depend on your own use of it.

## Limitation of Liability

To the fullest extent permitted by law, AlterX and its developers are not liable for any indirect, incidental, or consequential damages arising from your use of the app, including loss of data not backed up by you.

## Termination

You may stop using AlterX at any time by uninstalling it, which permanently deletes all associated data from your device (nothing is stored elsewhere to delete). We may update or discontinue the app at any time.

## Changes to These Terms

We may update these terms as AlterX changes. Continued use of the app after an update means you accept the revised terms. Material changes will be reflected here with an updated "Last updated" date.

## Contact

If you have questions about these terms, contact: **savannahgiddings1234@icloud.com**
